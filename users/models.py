...
class Developer(AbstractUser):
    ...
    position = models.CharField(
        max_length=50, choices=POSITION_CHOICES, default="backend"
    )
    score = models.FloatField(default=0)
    tech_stack = models.CharField(max_length=255, blank=True)
    avg_projects_score = models.DecimalField(
    ...
    def set_avg_project_score(self):
        project_ids = ProjectMembership.objects.filter(user=self).values_list(
            "project_id", flat=True
        )
        avg_score = (
            Project.objects.filter(id__in=project_ids).aggregate(
                avg=Avg("score")
            )["avg"]
            or 0
        )
...
class DeveloperRatings(models.Model):
    ...
    user_added = models.ForeignKey(
        user_model, on_delete=models.CASCADE, related_name="given_user_ratings"
    )
