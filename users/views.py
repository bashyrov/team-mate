...
class RegisterView(FormView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "users:profile", kwargs={"user_pk": self.request.user.pk}
        )
...
class LeaderboardView(ListView):
    ...
    def get_queryset(self):
        ...
        qs = qs.annotate(avg_score=Avg("projects__score")).order_by(
            "-avg_score", "username"
        )
...
class DeveloperDetailView(DetailView):
    ...
    def get_queryset(self):
        return (
            get_user_model()
            .objects.filter(pk=self.kwargs["user_pk"])
...
class DeveloperUpdateView(UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "users:profile", kwargs={"user_pk": self.object.pk}
        )
...
class MyProjectListView(ListView):
    ...
    def get_queryset(self):
        qs = (
            ...
            | self.request.user.owned_projects.all()
        ).distinct()
        return qs
