...
class ProjectRating(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.rated_by.given_project_ratings.filter(
...
class Project(models.Model):
    ...
    domain = models.CharField(
        max_length=50, choices=DOMEN_CHOICES, default="technology"
    )
    development_stage = models.CharField(
        max_length=50, choices=DEVELOPMENT_STAGE_CHOICES, default="initiation"
    )
    deploy_url = models.CharField(max_length=255, blank=True)
    owner = models.ForeignKey(
        user_model, related_name="owned_projects", on_delete=models.CASCADE
    )
    open_to_candidates = models.BooleanField(default=False)
    uid = models.CharField(
        max_length=22, unique=True, default=generate_shortuuid, editable=False
    )
    ...
    members = models.ManyToManyField(
        user_model, through="ProjectMembership", related_name="projects"
    )
...
class ProjectMembership(models.Model):
    ...
    def __str__(self):
        return (
            ...
            f"{self.project.name} as {self.get_role_display()}"
        )
...
class ProjectOpenRole(models.Model):
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name="open_roles"
    )
...
class ProjectApplication(models.Model):
    ...
    role = models.ForeignKey(
        ProjectOpenRole, on_delete=models.CASCADE, related_name="applications"
    )
    created_at = models.DateTimeField(auto_now_add=True)
