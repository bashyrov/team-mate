...
class TaskForm(forms.ModelForm):
    ...
    def __init__(self, *args, project=None, **kwargs):
        ...
        if project:
            self.fields["assignee"].queryset = project.members.values_list(
                "user", flat=True
            )
        else:
            self.fields["assignee"].queryset = Task._meta.get_field(
                "assignee"
...
class ProjectSearchForm(forms.Form):
    ...
    development_stage = forms.ChoiceField(
        ...
        + list(Project.DEVELOPMENT_STAGE_CHOICES),
        required=False,
        label="",
        widget=forms.Select(
            attrs={"class": "form-control", "style": "margin-top: 15px;"}
...
class ProjectMembershipForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.project:
            self.fields["user"].queryset = Project.objects.get(
                id=self.project.id
            ).members.all()
