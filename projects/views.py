...
from .forms import (
    ...
    TaskSearchForm,
    ProjectApplicationSearchForm,
)
from projects.permission_mixins import (
    TaskPermissionRequiredMixin,
    ProjectPermissionRequiredMixin,
    ProjectRatingPermissionMixin,
    ApplicationPermissionRequiredMixin,
    MembershipPermissionRequiredMixin,
    BasePermissionMixin,
)
    ...
    def get_queryset(self):
        return Project.objects.select_related("owner").prefetch_related(
            models.Prefetch(
                ...
                ).prefetch_related("tags", "assignee", "created_by"),
            ),
            models.Prefetch(
                "ratings",
                queryset=ProjectRating.objects.order_by(
                    "-created_at"
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated:
            ...
            if membership:
                ...
                user_permissions = {
                    perm: getattr(membership, perm, False)
                    for perm in permissions
                }

            is_rated = project.ratings.filter(
                project=project, rated_by=user
            ).exists()
...
class ProjectOpenRoleCreateView(ProjectPermissionRequiredMixin, CreateView):
    ...
    def form_invalid(self, form):
        if self.request.headers.get("x-requested-with") == "XMLHttpRequest":
            html = render_to_string(
                self.template_name, {"form": form}, self.request
            )
    ...
    def get_success_url(self):
        return reverse_lazy(
            "projects:project_open_roles_list",
            kwargs={
...
class ProjectOpenRoleListView(BasePermissionMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.user.is_authenticated:
            ...
            if membership:
                permissions = ["manage_open_roles_perm"]
...
class ProjectOpenRoleDeleteView(ProjectPermissionRequiredMixin, DeleteView):
    ...
    def get_object(self, queryset=None):
        return get_object_or_404(
            ProjectOpenRole, pk=self.kwargs["role_pk"], project=self.project
        )
...
class TaskListView(ListView):
    ...
    def get_queryset(self):
        ...
        qs = (
            Task.objects.filter(project_id=self.kwargs["project_pk"])
            .select_related("assignee", "created_by", "project__owner")
...
class TaskDetailView(TaskPermissionRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        return (
            Task.objects.filter(pk=self.kwargs["task_pk"])
            .select_related("assignee", "created_by", "project__owner")
            .prefetch_related("tags")
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        update_task_perm = (
            ...
            or self.user.is_owner(self.project)
        )

        if not hasattr(self, "project"):
            self.project = get_object_or_404(
                Project, pk=self.object.project.pk
            )
...
class ProjectCreateView(CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "projects:project_detail", kwargs={"project_pk": self.object.pk}
        )
...
class ProjectMembershipUpdateView(ProjectPermissionRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "projects:project_detail", kwargs={"pk": self.object.pk}
        )
...
class ProjectRatingCreateView(ProjectRatingPermissionMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if self.request.headers.get("HX-Request"):
            return render(
                self.request,
                "projects/forms/project_rating_form.html",
    ...
    def get_success_url(self):
        return reverse_lazy(
            "projects:project_detail", kwargs={"project_pk": self.project.pk}
        )
...
class ProjectRolesUpdateView(ProjectPermissionRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            ...
            formset = ProjectMembershipFormSet(
                instance=self.object, form_kwargs={"project": self.object}
            )
...
class TaskCreateView(ProjectPermissionRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            ...
            kwargs={"project_pk": self.project.pk, "task_pk": self.object.pk},
        )
    ...
    def form_valid(self, form):
        ...
        if assignee:
            if not ProjectMembership.objects.filter(
                project=self.project, user=assignee
            ).exists():
                raise PermissionDenied(
                    "Assignee must be a member of this project."
                )
...
@method_decorator(login_required, name="dispatch")
class ProjectApplicationCreateView(
    ApplicationPermissionRequiredMixin, CreateView
):
...
class ProjectApplicationListView(MembershipPermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["search_form"] = ProjectApplicationSearchForm(
            initial={"username": self.user_username, "role": self.role}
        )
...
def application_approve(request, **kwargs):
    ...
    try:
        with transaction.atomic():
            ...
            messages.success(
                request,
                f"{application.user.username} has been added to the project!",
            )

    except Exception:
        messages.error(
            request, "An error occurred while approving the application."
        )
...
def application_reject(request, **kwargs):
    ...
    try:
        with transaction.atomic():
            ...
            messages.success(
                request,
                f"{application.user.username} application has rejected!",
            )

    except Exception:
        messages.error(
            request, "An error occurred while rejecting the application."
        )
