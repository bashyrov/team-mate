...
class ModelsTestCase(TestCase):
    ...
    def test_get_members(self):
        ...
        user_1 = user_model.objects.create(
            username="testuser1", email="<EMAIL>", password="<PASSWORD>"
        )
        user_2 = user_model.objects.create(
            username="testuser2", email="<EMAIL>", password="<PASSWORD>"
        )
        user_3 = user_model.objects.create(
            username="testuser3", email="<EMAIL>", password="<PASSWORD>"
        )
        user_4 = user_model.objects.create(
            username="testuser4", email="<EMAIL>", password="<PASSWORD>"
        )

        project = Project.objects.create(
            name="Test Project", description="A test project", owner=user_1
        )

        ProjectMembership.objects.create(
            project=project, user=user_2, role="DEV"
        )
        ProjectMembership.objects.create(
            project=project, user=user_3, role="LEAD"
        )
        ProjectMembership.objects.create(
            project=project, user=user_4, role="PM"
        )

        members = project.members.all()
        expected_members = user_model.objects.filter(
            pk__in=[user_1.pk, user_2.pk, user_3.pk, user_4.pk]
        )
    ...
    def test_update_avg_score(self):
        ...
        self.rater1 = user_model.objects.create(
            username="rater1", password="<PASSWORD>"
        )
        self.rater2 = user_model.objects.create(
            username="rater2", password="<PASSWORD>"
        )

        self.assertEqual(project.score, 0)

        ProjectRating.objects.create(
            project=project, rated_by=self.user, score=4
        )
        ProjectRating.objects.create(
            project=project, rated_by=self.rater1, score=5
        )
        ProjectRating.objects.create(
            project=project, rated_by=self.rater2, score=3
        )
    ...
    def test_has_permission(self):
        ...
        user_test = user_model.objects.create(
            username="dev", email="dev@mail.com", password="pass"
        )
    ...
    def test_projectmembership_str(self):
        ...
        membership_obj = (
            ...
            .filter(project=project, user=self.user)
            .first()
        )
    ...
    def test_project_open_role_str(self):
        ...
        role = ProjectOpenRole.objects.create(project=project, role_name="DEV")
