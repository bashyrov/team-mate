...
class PermissionTest(TestCase):
    def setUp(self):
        ...
        self.user_owner = self.user_model.objects.create_user(
            username="owner", email="<EMAIL>", password="ownerpass"
        )
        ...
        self.update_project_stage_perm_user = (
        ...
        self.non_member = self.user_model.objects.create_user(
            username="non_member", email="<EMAIL>", password="nonmemberpass"
        )
        self.assigned_to_task = self.user_model.objects.create_user(
            username="assigned_to_task", email="<EMAIL>", password="memberpass"
        )
    ...
    def test_update_project_stage_permission_user(self):
        membership = ProjectMembership.objects.get(
            project=self.project, user=self.update_project_stage_perm_user
        )
    ...
    def test_manage_open_roles_permission_user(self):
        membership = ProjectMembership.objects.get(
            project=self.project, user=self.manage_open_roles_perm_user
        )
    ...
    def test_non_member_has_no_permissions(self):
        membership = ProjectMembership.objects.filter(
            project=self.project, user=self.non_member
        ).first()
